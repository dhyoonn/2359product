...
[분석 데이터]
- 플랫폼: 올리브영${productName ? `\n- 제품명: ${productName}` : ''}
- 총 리뷰 수: ${reviews.length}개 / 평균 별점: ${avgRating}점
- 별점 분포: ${ratingCounts.map((r) => `${r.score}점 ${r.count}개`).join(', ')}
...
2. 주요 장점 최대 5개: 반복 언급된 긍정 요소. 키워드 + 설명 + 대표 리뷰 인용 1~2개
3. 주요 단점 최대 5개: 반복 언급된 부정 요소. 키워드 + 설명 + 대표 리뷰 인용 1~2개
